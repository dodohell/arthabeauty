	...
	class Menus extends Settings{
		...
		function addEditRow($params){
			...
			$fields = array(
				...
				'is_faq'                => isset($params['is_faq']) ? 1 : 0,
			...
			if($act == "add") {
				...
				$res = $db->autoExecute($static_info_table,$fields,DB_AUTOQUERY_INSERT);
				safeCheck($res);
			...
			if($act == "edit") {
				$id = (int)$params["id"];
				$res = $db->autoExecute($static_info_table,$fields,DB_AUTOQUERY_UPDATE,"id = " . $id);
				safeCheck($res);
			}
