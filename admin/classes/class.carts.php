<?php
	class Carts extends Settings{
        ...
        public static function getCountNewOrders(){
            ...
            $countNewCartOrders = self::getCountNewCartOrders();
            $countNewFastOrders = self::getCountNewFastOrders();

            return $countNewCartOrders+$countNewFastOrders;
        }

        public static function getCountNewCartOrders(){
            global $db;
			global $carts_table;
            ...
            $row = $db->getRow("SELECT
                                ...
                                WHERE
                                    edate = 0
                                AND (status = 1 OR status = 11)
                                AND finalised = 1 "); safeCheck($row);
            return $row["cntr"] > 0 ? (int)$row["cntr"] : 0;
        }

        public static function getCountNewFastOrders(){
            global $db;
...
//            }
            ...
            if($cart["pharmacy_id"] > 0){
                ...
                $receiverSiteId = Delivery::getCourierSiteId((int)$cart["delivery_type_id"], $cart["delivery_region"], $cart["delivery_city"]);
//                echo "<pre>";
//                var_dump($cart);
////                var_dump($cart["delivery_type_id"]);
////                var_dump($cart["delivery_region"]);
////                var_dump($cart["delivery_city"]);
//                echo "</pre>";
//                exit();
		...
		function getCarts($page = 0, $limit = 50, $params, $from_user = 0){
            ...
            if ( $params->has("Filter") || $from_user ){
				...
				if ( $user_id || $from_user ){
                    if($from_user){
                        $sql_where .= " AND carts.user_id = '".$from_user."'";
                        $sm->assign("user_id", $from_user);
                    }else{
			...
			$sql = "SELECT carts.*,
						...
						*/
						...
						{$sql_order_status}
						{$sql_phone}
						ORDER BY carts.order_timestamp DESC
						LIMIT {$start}, {$limit}";
