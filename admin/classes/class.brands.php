...
class Brands extends Settings {
    ...
    public function addEditRow($params) {
        ...
        $id = $params->getInt("id");
        $manifacturer_id = $params->getInt('manifacturer_id');
        ...
        $fields = array(
            ...
            'manifacturer_id'   => $manifacturer_id > 0 ? $manifacturer_id : 0,
        ...
        if ($act == "add") {
            ...
            shiftPos($db, $brands_table);
            $res = $db->autoExecute($brands_table, $fields, DB_AUTOQUERY_INSERT); safeCheck($res);
        ...
        if ($act == "edit") {
            ...
            $id = $params->getInt("id");
            $res = $db->autoExecute($brands_table, $fields, DB_AUTOQUERY_UPDATE, "id = " . $id); safeCheck($res);
        }

        $htaccessUpdate = new Settings();
        $htaccess_type = "brands";
    ...
    public function deleteField($id, $field) {
        ...
        $res = $db->autoExecute($brands_table, array($field => ""), DB_AUTOQUERY_UPDATE, " id = '" . $id . "' ");
        safeCheck($res);

        return $res;
    }

    public function getBrands($activeOnly = 0) {
        global $db;
        global $lng;
        global $brands_table;
        ...
        $items = $db->getAll("SELECT
                                ...
                                WHERE
                                    b.id = p.brand_id
                                AND p.edate = 0) AS products_count
                            FROM
                                ".$brands_table." AS b
                            WHERE
                                b.edate = 0
                            {$activeOnlySQL}
                            ORDER BY b.pos"); safeCheck($items);
        return $items;
    }
    ...
    public function deleteRecord($id) {
        ...
        $res = $db->autoExecute($brands_table, $fields, DB_AUTOQUERY_UPDATE, " id = '" . $id . "' ");
        safeCheck($res);
