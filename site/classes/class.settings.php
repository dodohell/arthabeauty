...
//        }
        ...
        if ($check["record_id"] && !$language_check) {
            ...
            if ($check["type"] == "categories") {
                ...
                $categoriesPage->getPage($check["record_id"], $page, $params);
						...
						if ($check["type"] == "brands") {
                $brandsObj = new Brands();
                //$categoriesPage->getPage($check["record_id"], $page, $options);
                $brandsObj->getPage($params, $check['record_id']);
            }
            if ($check["type"] == "collections") {
                $collectionObj = new Collections();
                //$categoriesPage->getPage($check["record_id"], $page, $options);
                $collectionObj->getPage($params, $check['record_id']);
            }
    ...
    function getIndexPage() {
		...
		$new_products = Products::getNewOffers(6);
...
//        $sm->assign("special_offers", $special_offers);
    ...
    public function getNumberOfProductsBySearchString($search_string, $limit = 4, $returnType = 3) {
        global $db;
        global $lng;
        global $user;
        ...
        $sql_where = "";
        if ($search_string) {
            $sql_where .= " AND (MATCH(products.name_en,products.name_{$lng}) AGAINST('{$search_string}') OR products.name_en LIKE '%{$search_string}%' OR products.name_{$lng} LIKE '%{$search_string}%' OR products.brand_id IN (SELECT id FROM {$brands_table} WHERE name_{$lng} LIKE '%{$search_string}%') OR products.barcode LIKE '%{$search_string}%' )";
        }
