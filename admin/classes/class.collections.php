...
class Collections extends Settings {
    ...
    public function getCollections($params = Array()) {
        ...
        if ($params->getInt("activeOnly")) {
            $activeOnlySQL = " AND active = 1 ";
        }
        
        if ($params->getInt("brand_id")) {
            $brandSQL = " AND brand_id= ".$params->getInt("brand_id");
        }
        ...
        $items = $db->getAll("SELECT
                                ...
                                (SELECT COUNT(p.id) AS cntr FROM {$products_table} AS p WHERE c.id = p.collection_id AND p.edate = 0) AS prod_count
                            FROM
                                ".$collections_table." AS c
                            WHERE
                                edate = 0
