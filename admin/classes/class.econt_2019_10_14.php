	...
	class econt{
		...
		function createRequest($requestType = "cities", $filename = "econt_cities.xml", $filename_response = "econt_cities_response.xml", $cms = 0, $delivery_tracking_number = 0){
			...
			if ( $requestType == "shipping_request" ){
				...
				$data = array( 'xml' => file_get_contents($_SERVER["DOCUMENT_ROOT"].$dir_prefix."/".$filename));
...
//                $r = curl_exec($ch);
				...
				if ( @filemtime($filename) <= time() - 2592000 ) {
                    ...
                    $response = curl_exec($ch);
                    if($response === false) throw new Exception("Connection error.");
...
//					$response = curl_exec($ch);
		...
		function getOffices(){
			...
			$this->createRequest("offices", $filename, $filename_response, 1);
		...
		function createShippingRequest($cart_id, $receiver, $tariffSubCodeSecond, $amount, $weight, $cms = 0, $order_description = "", $free_delivery = 0, $payment_type = 1){
			global $econtSender;
            global $econtShipmentType;
            global $econtPackCount;
            global $econtShipmentInfo;
            global $econtPaymentInfo;
            global $default_office_code;
            
            $totalCartSum = $amount;
			$econtShipment = $econtShipmentInfo;
            $econtPayment = $econtPaymentInfo;
            ...
            foreach ($econtSender as $v) {
                if((int)$v["OfficeCode"] === (int)$default_office_code){
                    $sender = $v;
                }
            }
            if(!$sender){
                throw new Exception("You are trying to create a shipping request without a sender information!");
            }
			...
			$shipmentType = $econtShipmentType; //тип на пратката. Възможни стойности: PACK - колет, DOCUMENT - документи, DOCUMENTPACK - палет + документи
			...
			$packCount = $econtPackCount; // брой пакети
			...
			$paymentCDNumber = $econtPayment["paymentCDNumber"];
			
			// Ако страната на платеца е "Подател" проверяваме за безплатна доставка
			$isFreeDelivery = CartDiscounts::isFreeDelivery($totalCartSum, $shipmentWeight);
            if($isFreeDelivery){
                $paymentSide = "SENDER";
                $paymentShareSum = 0;
