...
//            $row["bonus_points_won"] = round($row["price"],0);
        ...
        public function deleteProductCommentRecord($id) {
            global $db;
        ...
        public function approveComment($id, $value) {
            global $db;
            global $products_comments_table;
        ...
        public function getProductsComments($options = array()) {
            global $db;
            global $products_comments_table;
            ...
            if ($options["product_id"]) {
                $sql_where .= " AND products_comments.product_id = '" . $options["product_id"] . "' ";
            }

            if ($options["for_approval"] === 1) {
                $sql_where .= " AND products_comments.approved = 0 ";
            }

            $userObj = new Users();
            $pages = $db->getRow("SELECT
                                    count(products_comments.id) AS cntr 
                                FROM 
                                    " . $products_comments_table . " AS products_comments
                                    {$sql_where_from}
                                WHERE 
                                    edate = 0 
            ...
            if ($page > 0) {
                $generate_pages .= '<li class="page-item first"><a href="products_comments.php?' . $_SERVER["QUERY_STRING"] . '&page=0" class="page-link">First</a></li>';
            } else {
                $generate_pages .= '<li class="page-item first active"><span class="page-link">First</span></li>';
            }
            if ($page > 0) {
                $generate_pages .= '<li class="page-item"><a href="products_comments.php?' . $_SERVER["QUERY_STRING"] . '&page=' . ($page - 1) . '" class="page-link">Previous</a></li>';
            } else {
                $generate_pages .= '<li class="page-item"><span class="page-link">Previous</span></li>';
            }
            ...
            for ($i = $start_page; $i < $end_page; $i++) {
                if ($page == $i) {
                    ...
                    $generate_pages .= '<li class="page-item"><a class="page-link" href="products_comments.php?' . $_SERVER["QUERY_STRING"] . '&page=' . $i . '">' . ($i + 1) . '</a></li>';
                }
            }
            //$generate_pages .= '</span>';
            ...
            if ($page < $total_pages - 1) {
                ...
                $generate_pages .= '<li class="page-item"><span class="page-link" >Next</span></li>';
            }

            if ($page < $total_pages - 1) {
                $generate_pages .= '<li class="page-item last"><a class="page-link" href="products_comments.php?' . $_SERVER["QUERY_STRING"] . '&page=' . ($total_pages - 1) . '">Last</a></li>';
            } else {
                $generate_pages .= '<li class="page-item last"><span class="page-link">Last</span></li>';
            }
            ...
            $results = $db->getAll("SELECT
                                    ...
                                    LIMIT {$start}, {$limit}"); safeCheck($results);
