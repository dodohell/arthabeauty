<?php
	...
	$id = $_REQUEST["id"];
		...
		$user_info = $db->getRow("SELECT * 
								...
								WHERE cart_id = '".$tmp_cart["id"]."' ORDER BY id DESC"); safeCheck($user_info);
		...
		if ( $tmp_cart["delivery_type_id"] == 2 || $tmp_cart["delivery_type_id"] == 4 ){
            try {
                $offices_tmp = Delivery::getOfficesByRegionAndCityNames((int)$tmp_cart["delivery_type_id"], $tmp_cart["delivery_region"], $tmp_cart["delivery_city"]);
                echo "<pre>";
                var_dump($offices_tmp);
                echo "</pre>";
                exit();
            }catch (Exception $e) {
                $file = __FILE__; $line = __LINE__;
                $speedyError = $e->getMessage();
                $res = $db->autoExecute($error_log_delivery_table, array("carts_id" => $cart_id, "file" => $file, "line" => $line, "message" => $speedyError, "server_info" => serialize($_SERVER), "error_datetime" => date("Y-m-d H:i")), DB_AUTOQUERY_INSERT); safeCheck($res);
		...
		<table cellpadding="0" width="100%"><tr><td width="45%" valign="top">
			...
			'.$user_info["delivery_city"].', '.$user_info["delivery_region"].'<br />
			...
			'.$user_info["email"].'<br />
			'.$user_info["billing_city"].', '.$user_info["billing_region"].'<br />
